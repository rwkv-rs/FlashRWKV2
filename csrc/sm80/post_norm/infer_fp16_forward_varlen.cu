...
std::vector<at::Tensor> empty_like_pack(
    const at::Tensor& reference, int64_t count) {
  auto storage = at::empty({count * reference.numel()}, reference.options());
...
at::Tensor ln_forward_varlen_cuda(at::Tensor x, at::Tensor weight, at::Tensor bias, double eps) {
  ...
  auto stream = at::cuda::getCurrentCUDAStream();
  if (FLASHRWKV_TARGET_SM == 120 && C == LN_SMALL_C) {
    if (rows >= 1024) {
...
std::vector<at::Tensor> res_ln_f16_cuda(at::Tensor x, at::Tensor res, at::Tensor weight, at::Tensor bias, double eps) {
  ...
  if (FLASHRWKV_TARGET_SM == 120 && C == LN_SMALL_C && rows < 1024) {
...
std::vector<at::Tensor> post_norm_forward_varlen_cuda(
  ...
  const int64_t rows = x.numel() / C;
  if (FLASHRWKV_TARGET_SM == 120 && C == LN_SMALL_C && batch_size >= 2 &&
      batch_size <= rows && rows <= 1024) {
...
at::Tensor post_norm_output_forward_varlen_cuda(
  ...
  if (FLASHRWKV_TARGET_SM == 120 && C == LN_SMALL_C && rows < 1024) {
    res_ln_f16_small_kernel<LN_SMALL512_THREADS / 2, true, true>
        <<<static_cast<int>(rows), LN_SMALL512_THREADS / 2, 0, stream>>>(
            x.data_ptr<dtype>(), res.data_ptr<dtype>(), weight.data_ptr<dtype>(),
            bias.data_ptr<dtype>(), output.data_ptr<dtype>(),
            output.data_ptr<dtype>(), rows, static_cast<float>(eps));
  } else if (FLASHRWKV_TARGET_SM == 120 && C == LN_SMALL_C) {
    res_ln_f16_small_kernel<LN_SMALL512_THREADS, true, true>
        <<<static_cast<int>(rows), LN_SMALL512_THREADS, 0, stream>>>(
            x.data_ptr<dtype>(), res.data_ptr<dtype>(), weight.data_ptr<dtype>(),
            bias.data_ptr<dtype>(), output.data_ptr<dtype>(),
    ...
    res_ln_f16_kernel<<<static_cast<int>(rows), LN_THREADS, 0, stream>>>(
        C, x.data_ptr<dtype>(), res.data_ptr<dtype>(), weight.data_ptr<dtype>(),
        bias.data_ptr<dtype>(), output.data_ptr<dtype>(), output.data_ptr<dtype>(),
