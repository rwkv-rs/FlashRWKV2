...
std::vector<torch::Tensor> pretrain_tmix_tokenshift_backward_cuda(
    ...
    auto parameter_storage = torch::empty({6, x.size(2)}, x.options());
    auto parameter_grads = parameter_storage.unbind(0);
    auto fp32_opts = x.options().dtype(torch::kFloat);
    auto accumulator_storage = torch::zeros({6, x.size(2)}, fp32_opts);
