...
__device__ inline void atomic_add_float2(float* ptr, float2 value) {
  atomicAdd(ptr, value.x);
  atomicAdd(ptr + 1, value.y);
}
...
std::vector<torch::Tensor> statetune_tmix_tokenshift_backward_cuda(
  ...
  auto accumulator_storage = torch::zeros(
      {6, x.size(2)}, x.options().dtype(torch::kFloat32));
  auto accumulators = accumulator_storage.unbind(0);
  auto coefficient_storage = torch::empty({6, x.size(2)}, x.options());
  auto coefficient_grads = coefficient_storage.unbind(0);
