...
// 3e41bc43ed5e8332927ddd7e0ce4816cf200a6ea.  The binding below owns the
// FlashRWKV2 packed-varlen, slot-bundle, and fail-closed contracts.
...
void check_deltalog_layout(
    ...
    const torch::stable::Tensor& elapsed_pool,
  ...
  for (const auto& item : {
           ...
           {&elapsed_pool, "elapsed_state_pool"},
  ...
  STD_TORCH_CHECK(
      query_start_loc.scalar_type() == torch::headeronly::ScalarType::Int &&
          ...
          elapsed_pool.scalar_type() == torch::headeronly::ScalarType::Int &&
          phase_pool.scalar_type() == torch::headeronly::ScalarType::Int,
      "DeltaLog metadata, elapsed, and phase tensors must be int32");
  STD_TORCH_CHECK(
  ...
  STD_TORCH_CHECK(
      state_pool.dim() == 4 && state_pool.scalar_type() == torch::headeronly::ScalarType::Half &&
  ...
  STD_TORCH_CHECK(
      elapsed_pool.dim() == 1 &&
          elapsed_pool.size(0) == state_pool.size(0),
  ...
  STD_TORCH_CHECK(
      deltalog_pool.dim() == 5 &&
          ...
          deltalog_pool.scalar_type() == torch::headeronly::ScalarType::Half,
  ...
  STD_TORCH_CHECK(
      ...
      "DeltaLog token tensors must be float16");
...
void tmix_wkv7_recurrent_deltalog_fp16_from_decay_logits(
  ...
  check_deltalog_layout(
      ...
      deltalog_pool, r, decay_logits, k, v, a, b, output, scale);
  ...
  auto deltalog_status = deltalog_status_workspace.value_or(torch::stable::Tensor());
  if (deltalog_status.defined()) {
    check_cuda_contiguous(deltalog_status, "deltalog_status_workspace");
    check_same_device(
        state_pool, deltalog_status, "deltalog_status_workspace");
    STD_TORCH_CHECK(
        deltalog_status.scalar_type() == torch::headeronly::ScalarType::Int &&
            deltalog_status.dim() == 1 &&
            deltalog_status.numel() == 1 + 2 * state_indices.numel(),
        "deltalog_status_workspace must be int32 [1+2*sequence_capacity]");
  } else {
    deltalog_status = torch::stable::new_empty(
        state_pool, {1 + 2 * state_indices.numel()},
        torch::headeronly::ScalarType::Int);
  }
  tmix_wkv7_recurrent_deltalog_fp16_from_decay_logits_cuda(
      query_start_loc, state_indices, elapsed_pool, phase_pool,
...
STABLE_TORCH_LIBRARY_FRAGMENT(flashrwkv2, module) {
  module.def("tmix_wkv7_recurrent_deltalog_fp16_from_decay_logits(Tensor query_start_loc, Tensor state_indices, Tensor(a!) elapsed_state_pool, Tensor(b!) deltalog_phase_pool, Tensor(c!) state_pool, Tensor(d!) deltalog_pool, Tensor r, Tensor decay_logits, Tensor k, Tensor v, Tensor a, Tensor b, Tensor(e!) output, float scale, Tensor? decay_bias, Tensor metadata_status, Tensor? deltalog_status_workspace) -> ()");
  module.def("tmix_wkv7_recurrent_deltalog_fp16_materialize_slots(Tensor state_indices, Tensor(a!) deltalog_phase_pool, Tensor(b!) state_pool, Tensor(c!) deltalog_pool, Tensor(d!) deltalog_status_workspace, Tensor? metadata_status) -> ()");
}

STABLE_TORCH_LIBRARY_IMPL(flashrwkv2, CUDA, module) {
  module.impl("tmix_wkv7_recurrent_deltalog_fp16_from_decay_logits", TORCH_BOX(&tmix_wkv7_recurrent_deltalog_fp16_from_decay_logits));
  module.impl("tmix_wkv7_recurrent_deltalog_fp16_materialize_slots", TORCH_BOX(&tmix_wkv7_recurrent_deltalog_fp16_materialize_slots));
}
