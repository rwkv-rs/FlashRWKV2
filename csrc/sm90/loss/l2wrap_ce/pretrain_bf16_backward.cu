...
torch::stable::Tensor l2wrap_ce_backward_cuda(
    ...
    auto grad_logits = torch::stable::empty_like(logits);
    auto stream = flashrwkv2::validation::current_cuda_stream();
    constexpr int threads = 512;
    if (logits.scalar_type() == torch::headeronly::ScalarType::BFloat16) {
        launch_l2wrap_ce_backward_v2_kernel<torch::headeronly::BFloat16, threads>(
            grad_loss.mutable_data_ptr<float>(),
            ...
            grad_logits.mutable_data_ptr<torch::headeronly::BFloat16>(),
            rows,
            vocab,
            stream);
    } else {
        launch_l2wrap_ce_backward_v2_kernel<float, threads>(
            ...
            max_vals.mutable_data_ptr<float>(),
            argmax.mutable_data_ptr<int>(),
            grad_logits.mutable_data_ptr<float>(),
            rows,
            vocab,
            stream);
    }
