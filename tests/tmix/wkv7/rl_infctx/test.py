...
def test_rl_infctx_ragged_materialized_and_recompute(d: int) -> None:
    ...
    materialized_output, materialized_pool = rl_infctx_tmix_wkv7_chunk_fp32io16(
        *inputs,
        state_pool=pool,
        cu_seqlens=cu,
        state_indices=slots,
    ...
    recompute_output, recompute_pool = rl_infctx_tmix_wkv7_chunk_fp32io16(
        *inputs,
        state_pool=pool,
        cu_seqlens=cu,
        state_indices=slots,
...
def test_rl_infctx_bf16_chunk_sizes_and_tail_match_reference(d: int) -> None:
    ...
    for chunk_size in (16, 32, 64):
        for strategy in ("materialized", "recompute"):
            output, final_pool = rl_infctx_tmix_wkv7_chunk_fp32io16(
                *inputs,
                state_pool=pool,
                cu_seqlens=cu,
                state_indices=slots,
